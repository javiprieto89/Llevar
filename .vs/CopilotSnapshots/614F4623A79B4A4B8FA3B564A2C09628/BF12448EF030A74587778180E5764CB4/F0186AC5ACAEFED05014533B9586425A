...
# Importar módulo completo con funciones auxiliares
Import-Module "Q:\Utilidad\LLevar\Modules\Core\TransferConfig.psm1" -Force
...
try {
    ...
    Write-Host "  [DEBUG] Destino.Tipo: '$($llevar.Destino.Tipo)' (esperado vacío)" -ForegroundColor DarkGray
    Write-Host "  [DEBUG] Opciones.BlockSizeMB: $($llevar.Opciones.BlockSizeMB) (esperado 10)" -ForegroundColor DarkGray
    
    if ([string]::IsNullOrEmpty($llevar.Destino.Tipo) -and $llevar.Opciones.BlockSizeMB -eq 10) {
        ...
        Write-Host "  Destino.Tipo actual: '$($llevar.Destino.Tipo)'" -ForegroundColor Red
        Write-Host "  BlockSizeMB actual: $($llevar.Opciones.BlockSizeMB)" -ForegroundColor Red
        exit 1
    }
...
try {
    # TODO: Test-TransferConfigComplete no implementada aún
